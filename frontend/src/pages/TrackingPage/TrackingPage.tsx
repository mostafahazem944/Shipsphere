...
export default function Tracking() {
  ...
  if (!trackingNumber) {
    return (
      <div className="space-y-6">
        <Breadcrumb items={[{ label: 'Dashboard', href: '/user' }, { label: 'Track Shipment' }]} />
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6">No shipment ID provided</p>
  ...
  const currentLocation =
    tracking?.events?.[0]?.location ?? currentShipment?.senderAddress.city ?? '—';
      ...
      : [
          {
            ...
            location: currentShipment?.senderAddress.city || '—',
            date: currentShipment?.createdAt || '—',
            completed: true,
            current: true
          }
        ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Track Your Shipment</h1>
      ...
      <div className="bg-blue-600 dark:bg-blue-700 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-blue-200 mt-1">Shipped via {courierName}</p>
          </div>
        ...
        <div>
          ...
          <p className="text-sm text-blue-200 mt-2">
            Current location: <span className="font-semibold text-white">{currentLocation}</span>
          </p>
        </div>
      </div>
        ...
        <div className="space-y-5">
          <Card>
            ...
            <div className="space-y-3 text-sm">
              ...
              {currentShipment?.package && (
                <div className="space-y-2">
                  {/* Weight Row - FIX: Displaying correct weight unit (kg/lb) */}
                  {currentShipment.package.weight && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 dark:text-gray-400">Weight</span>
                      <span className="font-medium text-gray-900 dark:text-gray-100">
                        {currentShipment.package.weight}{' '}
                        {currentShipment.package.units === 'cm' ? 'kg' : 'lb'}
                      </span>
                    </div>
                  )}

                  {/* Dimensions Row (Length x Width x Height) */}
                  {(currentShipment.package.width ||
                    currentShipment.package.height ||
                    currentShipment.package.length) && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 dark:text-gray-400">Dimensions</span>
                      <span className="font-medium text-gray-900 dark:text-gray-100">
                        {currentShipment.package.length} × {currentShipment.package.width} ×{' '}
                        {currentShipment.package.height} {currentShipment.package.units}
                      </span>
                    </div>
                  )}
                </div>
