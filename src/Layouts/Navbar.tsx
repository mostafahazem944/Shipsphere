...
const Navbar = () => {
  ...
  return (
    <>
      <div className={`fixed w-full top-0 z-50 flex justify-center transition-all duration-500 ${scrolled ? "pt-2" : "pt-6"}`}>
        <nav
        ...
        >
            ...
            {mainLinks.map((link) => (
              <NavLink key={link.name} to={link.path} className={linkStyle}>
                {({ isActive }) => (
                  <>
                    {link.name}
                    <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 transform origin-left transition-transform duration-300 ${isActive ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`} />
                  </>
                )}
              </NavLink>
            ))}
          ...
          <div className="flex items-center gap-4 flex-1 justify-end">
            ...
            <button
              ...
              className="md:hidden p-2 text-slate-950 dark:text-white bg-gray-100 dark:bg-gray-800 rounded-lg"
            >
        ...
        <div className={`absolute right-4 top-24 left-4 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-8 rounded-3xl shadow-2xl transition-all duration-300 ${isOpen ? "translate-y-0 opacity-100" : "-translate-y-10 opacity-0"}`}>
          <div className="flex flex-col gap-6">
            <div className="space-y-4">
              ...
              {mainLinks.map((link) => (
                <NavLink key={link.name} to={link.path} onClick={() => setIsOpen(false)} className="block text-2xl font-black text-slate-950 dark:text-white">
                  {link.name}
                </NavLink>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-4">
                <button onClick={() => {navigate("/login"); setIsOpen(false)}} className="flex-1 py-3 text-sm font-bold border border-gray-200 dark:border-gray-700 rounded-xl text-slate-950 dark:text-white">
                  Login
                </button>
                <button onClick={() => {navigate("/signup"); setIsOpen(false)}} className="flex-1 py-3 text-sm font-bold bg-slate-950 dark:bg-white dark:text-black text-white rounded-xl">
                  Sign Up
                </button>
              </div>
            </div>
